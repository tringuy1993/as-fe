...
export default function Home() {
  ...
  //Request ToS TheoData
  const params3 = req_params('$SPX.X', 'gamma', finalDate)
  const { data: ToSTheoData} = useFetch(params3, THEO_URL, update_param, updateInterval)

  const ToSTheoData_SPX = ToSTheoData?.map(data => GetModifiedToSTheoData(data, selectedGreek))
  ...
  if (ToSData?.data){
    ...
    EChartToSList = tickersGraphs2.map(({ symbol, data, theoData }) => (
      <EChartToS key={symbol} symbol={symbol} data={data} theoData={theoData} greek={selectedGreek} />
    ));
